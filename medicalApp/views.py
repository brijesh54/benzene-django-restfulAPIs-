...
from .models import Company, CompanyBank, Medicine, MedicineDetails, CompanyAccount, Employee, \
    EmployeeBank, EmployeeSalary, CustomerRequest, Bill, BillDetails
from .serializers import CompanySerliazer, CompanyBankSerializer, MedicineSerializer, \
    MedicineDetailsSerializer, MedicineDetailsSerializerSimple, CompanyAccountSerializer, EmployeeSerializer, \
...
class CompanyViewSet(viewsets.ModelViewSet):
    ...
    def create(self,request):
        try:
            serializer = CompanySerliazer(data = request.data,context = {"context":request})
            if Company.objects.filter(email=self.request.data['email']).exists():
                return Response({"error": "This email id already exists."})
            if Company.objects.filter(license_no=self.request.data['license_no']).exists():
                return Response({"error": "This License No already exists."})
            if Company.objects.filter(contact_no=self.request.data['contact_no']).exists():
                return Response({"error": "This contact_no already exists."})
            serializer.is_valid()
            serializer.save()
            response_dict = {"error":False,"message":"data added successfully"}   
            return Response(response_dict)  
        except:
            response_dict = {"error":True,"message":"error during added data"}    
        return Response(response_dict)
...
class CompanyBankViewSet(viewsets.ModelViewSet):
    ...
    def create(self,request):
        try:
            ...
            if CompanyBank.objects.filter(bank_account_no=self.request.data['bank_account_no']).exists():
                return Response({"error": "This bank account no already exists."})
            serializer.is_valid(raise_exception=True)
            serializer.save()
...
class MedicineViewSet(viewsets.ModelViewSet):
    ...
    def create(self,request):
        try:
            ...
            serializer2=MedicineDetailsSerializer(data=medicine_details_list,many=True,context={"request":request})
    ...
    def list(self,request):
        ...
        for medicine in medicine_data:
            ...
            medicine_details_serializers=MedicineDetailsSerializerSimple(medicine_details,many=True)
    ...
    def update(self,request,pk=None):
        ...
        for salt_detail in request.data["medicine_details"]:
            if salt_detail["id"]==0:
                ...
                serializer2 = MedicineDetailsSerializer(data=salt_detail,context={"request": request})
                serializer2.is_valid()
                serializer2.save()
            else:
                ...
                del salt_detail["id"]
                serializer3=MedicineDetailsSerializer(medicine_salt,data=salt_detail,context={"request":request})
...
company_update = CompanyViewSet.as_view({"update":"update"})
